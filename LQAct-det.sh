...
PY
    ...
    echo "Crashed detection variant ${variant_id}" >&2
    return 1
}
...
for MODEL in "${MODEL_NAME[@]}"; do
    for DATASET in "${DATASETS[@]}"; do
        ...
        run_variant "det:${DATASET}:${MODEL}:LoRA" "./outputs_det/${DATASET}_${MODEL}_LoRA" \
            --task_type object_detection \
            --model_name "${MODEL}" \
            --dataset_name "${DATASET}" \
            --data_dir "${DATA_DIR}" \
            --patch_locations ${PATCH_LOCATIONS} \
            --output_dir "./outputs_det/${DATASET}_${MODEL}_LoRA" \
            --wandb_project_name "DET-${DATASET}-${COMPRESS_METHOD}" \
        ...
        for RANK in "${RANKS[@]}"; do
            run_variant "det:${DATASET}:${MODEL}:loract:${RANK}" "./outputs_det/${DATASET}_${MODEL}_loract${RANK}" \
                --task_type object_detection \
                --model_name "${MODEL}" \
                --dataset_name "${DATASET}" \
                --data_dir "${DATA_DIR}" \
                --patch_locations ${PATCH_LOCATIONS} \
                --output_dir "./outputs_det/${DATASET}_${MODEL}_loract${RANK}" \
                --wandb_project_name "DET-${DATASET}-${COMPRESS_METHOD}" \
            ...
            run_variant "det:${DATASET}:${MODEL}:eras-sa:${RANK}" "./outputs_det/${DATASET}_${MODEL}_dk${RANK}_er${ENERGY_RATIO}" \
                --task_type object_detection \
                --model_name "${MODEL}" \
                --dataset_name "${DATASET}" \
                --data_dir "${DATA_DIR}" \
                --patch_locations ${PATCH_LOCATIONS} \
                --output_dir "./outputs_det/${DATASET}_${MODEL}_dk${RANK}_er${ENERGY_RATIO}" \
                --wandb_project_name "DET-${DATASET}-${COMPRESS_METHOD}" \
            ...
            run_variant "det:${DATASET}:${MODEL}:eras-eb:${RANK}" "./outputs_det/${DATASET}_${MODEL}_dk${RANK}_es" \
                --task_type object_detection \
                --model_name "${MODEL}" \
                --dataset_name "${DATASET}" \
                --data_dir "${DATA_DIR}" \
                --patch_locations ${PATCH_LOCATIONS} \
                --output_dir "./outputs_det/${DATASET}_${MODEL}_dk${RANK}_es" \
                --wandb_project_name "DET-${DATASET}-${COMPRESS_METHOD}" \
