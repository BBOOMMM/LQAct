...
from ..functions import t5_rms_norm_forward, nn_linear_forward, llama_mlp_forward, t5_rms_norm_forward_lowrank_plus_quantization
...
def apply_patch_to_llama_model(
    ...
    for layer in base_model.layers:
        ...
        if compress_kwargs.get("lowrank_plus_quantization", False):
            ...
            if norm:
                _patch_module(layer.input_layernorm, t5_rms_norm_forward_lowrank_plus_quantization, compress_kwargs=compress_kwargs)
                _patch_module(layer.post_attention_layernorm, t5_rms_norm_forward_lowrank_plus_quantization, compress_kwargs=compress_kwargs)
            if attn_in:
                _patch_module(layer.self_attn.q_proj, nn_linear_forward, compress_kwargs=compress_kwargs)
                _patch_module(layer.self_attn.k_proj, nn_linear_forward, compress_kwargs=compress_kwargs)
                _patch_module(layer.self_attn.v_proj, nn_linear_forward, compress_kwargs=compress_kwargs)
