...
def probing_qb(
    ...
    assert isinstance(prob_nums, int | None)
    assert isinstance(energy_threshold, float)
    ...
    probe_indices = torch.randperm(m)[:prob_nums]
    A_sample = A[probe_indices, :]  # (S, n)

    G = A_sample @ A_sample.T  # (S, S)
    
    L = torch.linalg.eigvalsh(G.to(torch.float32))  # (S,)
    L = L.flip(dims=(0,))
    ...
    k_opt = torch.where(cumulative_energy >= threshold_energy)[0][0].item() + 1

    # print(f"selected rank {k_opt}, original rank {rank}")
    ...
    if (left is True) or (left is None and m <= n):
        ...
        QT, BT = probing_qb(A.mT, rank, niter=niter, test_matrix=test_matrix, left=True)
        return BT.mT, QT.mT
