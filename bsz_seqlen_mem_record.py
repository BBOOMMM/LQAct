...
def main():
    ...
    if args.vision:
        ...
        if args.compress_rank == 1:
            save_name = f"loract/language/{loc_prefix}_ckpt_{args.micro_batch_size}_{args.cutoff_len}"
        else:
            save_name=f"loract/language/{loc_prefix}_{args.compress_rank}_{args.micro_batch_size}_{args.cutoff_len}"
    ...
    def tokenize(prompt, add_eos_token=True,cutoff_len=args.cutoff_len):
    ...
    def generate_and_tokenize_prompt(data_point,train_on_inputs=True,add_eos_token=True,cutoff_len=args.cutoff_len):
    ...
    def trace_handler(prof: torch.profiler.profile):
        ...
        prof.export_memory_timeline(f"./bsz_seqlen_profiler/{file_prefix}.html", device="cuda:0")
        prof.export_memory_timeline(f"./bsz_seqlen_profiler/{file_prefix}.json", device="cuda:0")
    ...
    with open(f"./bsz_seqlen_profiler/{file_prefix}.json", "r") as f:
