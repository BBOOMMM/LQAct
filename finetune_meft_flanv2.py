...
def train(
    ...
    def generate_and_tokenize_prompt(data_point, train_on_inputs=True, add_eos_token=True,cutoff_len=2048):
        # 兼容不同数据集的字段名（instruction/input 或 user/assistant）
        instruction = data_point.get("instruction", "")
        # 兼容不同数据集的字段名（instruction/input 或 user/assistant）
        input_text = data_point.get("input", "")
        source_content = data_point.get("source", instruction)
        ...
        if "target" in data_point:
            target_content = data_point["target"]
        elif "output" in data_point:
            target_content = data_point["output"]
        else:
            target_content = ""
        ...
        # 如果不训练输入部分，掩码输入部分的labels
        if not train_on_inputs:
            user_prompt = prompter.generate_prompt(target_content, None, "")
            tokenized_user_prompt = tokenize(user_prompt, cutoff_len=cutoff_len, add_eos_token=add_eos_token)
            user_prompt_len = len(tokenized_user_prompt["input_ids"])
                
            if add_eos_token:
                user_prompt_len -= 1  # 减去EOS token的长度
    ...
    shuffled_dataset = data['train'].shuffle(seed=42)  # seed 固定，每次打乱结果一致
    # print(shuffled_dataset)

    # 3. 选择前 50k 个样本
    sample_size = 50000
    # 确保数据集总样本数 >= 50k，避免索引越界
    if len(shuffled_dataset) >= sample_size:
        selected_dataset = shuffled_dataset.select(range(sample_size))
    else:
        # 如果数据集不足 50k，全部选择（可选：重复采样，但不推荐）
        selected_dataset = shuffled_dataset
        print(f"警告：数据集仅包含 {len(shuffled_dataset)} 个样本，已全部选择")

    # 查看结果
    print(f"原始数据集大小：{len(data['train'])}")
    print(f"随机选择后大小：{len(selected_dataset)}")
...
def parse_args():
    ...
    parser.add_argument("--device_map", type=str, default="auto", help="Device map for model parallelism")
