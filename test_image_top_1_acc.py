...
huggingface_hub.login(token="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")  # 替换为你的实际token
print("登录成功!")
...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default="./eval/image/cifar100/meft_layer_0.25_compress_results",
                      help="评估结果输出目录")
...
def main(args):
    ...
    # 修改分类器层，使其与当前数据集的类别数匹配
    if args.data_path == "uoft-cs/cifar100":
        # CIFAR100 有 100 类
        merged_model.classifier = torch.nn.Linear(merged_model.classifier.in_features, 100)
    elif args.data_path == "ufldl-stanford/svhn":
        # SVHN 有 10 类
        merged_model.classifier = torch.nn.Linear(merged_model.classifier.in_features, 10)
    elif args.data_path == "ethz/food101":
        # Food101 有 101 类
        merged_model.classifier = torch.nn.Linear(merged_model.classifier.in_features, 101)
    ...
    def preprocess_function(examples):
        """预处理图像和标签，确保输出正确的张量格式"""
        # 处理图像
        images = examples[image_field]
        
        # 使用ViTImageProcessor的正确参数
        processed = processor(
            images=images,
    ...
    def collate_fn(batch):
        ...
        # 处理标签 - 修复'int' object is not subscriptable错误
        # 检测标签类型并正确处理
        labels = []
        for example in batch:
            ...
            if isinstance(label, (list, np.ndarray, torch.Tensor)):
                # 如果是列表等，取第一个元素
                if len(label) > 0:
                    labels.append(label[0] if not isinstance(label, torch.Tensor) else label.item())
                else:
